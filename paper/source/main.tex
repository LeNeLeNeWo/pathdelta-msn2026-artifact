...
The LLM chooses the implementation; applicable backends supply evidence on
which the obligation gate decides release and returns patch-free
counterexamples for repair.  In a frozen public-corpus challenge, goal/scope
checks accepted all 120 collateral candidates and a visible verifier accepted
12; \system rejected all 120 while accepting all 58 safe alternatives.  In a disjoint 40-change
...
Production network automation frequently centers on \emph{Day-2 changes}: incremental updates
...
from intent and pre-state a candidate-independent boundary for covered
non-target behavior.
...
The acceptance problem is to decide whether the \emph{entire covered} semantic delta
...
\end{equation*}
where $\target$ and $\semframe$ are target and preservation obligation sets,
$\deps$ and $\budget$ are structural-safeguard and footprint-budget obligation
sets, and $\Gamma$ is the epistemic
...
$b=(d,s,f,M,\rho)$ carries device, subject, FEC, attribute map, and provenance.
...
$t=(b,k,r,M_0[k],v)$ requiring relation $r$ between old value $M_0[k]$ and
...
obligation $f=(b,k,\textit{preserve},M_0[k])$ for every behavior-attribute pair
...
$Q\leftarrow$ one probe per nonempty exact/overlap/adjacent/default region\\
\hspace*{1.2em}(or applicable Batfish policy class); $X\leftarrow\operatorname{Evaluate}(N_0,Q)$;\\
$B^{\mathrm{cov}}\leftarrow B_0\cup X$;\\
...
$\Gamma\leftarrow$ hashes, provenance, applicability, and uncovered reasons;\\
...
diagnostic feedback reports affected behaviors and required relations, while
replacement text, object names, sequence numbers, and commands are forbidden.
...
ManualEnvelope and closes collateral left by visible verification.  The
...
release; the best-completing adapted prior workflow completes 18 and releases ten
...
decision.
The \href{https://anonymous.4open.science/r/pathdelta-msn2026-artifact-7998/}{\emph{anonymous artifact repository}} provides source and reproduction records
(the manuscript PDF is omitted).
